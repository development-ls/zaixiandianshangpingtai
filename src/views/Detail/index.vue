        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前咱们的路由跳转:从A路由直接跳转到B路由,但是这里在加入购物车,进行路由跳转之前，发请求把购买的产品信息通过请求的形式通知服务器,服务器进行相应的存储 -->
...
export default {
  ...
  methods: {
    ...
    async addShopCar() {
      //1.派发actions----将产品加入到数据库(通知服务器)
      //加入购物车的回调函数
      //要先判断加入购物车是成功了还是失败了
      //this.$store.dispatch实质上是在利用actions调用函数
      ...
      try {
        ...
        //进行路由的跳转
        //在路由跳转的同时还需要将产品的信息带给下一级的路由组件
        //直接在路由中将产品信息和产品id用query参数传递也可以实现,但是可能会导致地址栏的信息数据过多
        /* this.$router.push({
          name: "addcartsuccess",
          query: { skuInfo: this.skuInfo, skuNum: this.skuNum },
        }); */
        //复杂数据比如这里的skuInfo,可以通过会话存储(sessionStorage)---不持久化,会话结束数据消失
        //本地存储和会话存储都不能直接存储对象,只能存储字符串,通过JSON.stringify()将对象转化为字符串,取出数据时,在用JSON.parse()转换为对象
